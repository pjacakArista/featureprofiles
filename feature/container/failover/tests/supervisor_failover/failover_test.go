...
func TestContainerAndVolumePersistence(t *testing.T) {
	...
	t.Run("Setup", func(t *testing.T) {
		...
		startOpts := []client.StartOption{
			...
			// container after the supervisor switchover brings up Docker on the new
			// active. OCGNOIContainerzRedSup syncs the container definition to the
			// standby, but without this policy Docker leaves the container STOPPED.
...
func TestContainerPersistenceAfterColdReboot(t *testing.T) {
	...
	t.Run("Setup", func(t *testing.T) {
		...
		if s := deviations.ContainerzSwitchoverReadyTimeoutS(dut); s > 0 {
			switchoverReadyTimeout = time.Duration(s) * time.Second
		}
...
func verifyTimeoutForDUT(dut *ondatra.DUTDevice) time.Duration {
	if s := deviations.ContainerzPostSwitchoverVerifyTimeoutS(dut); s > 0 {
...
func awaitSwitchoverReadyAndSwitch(t *testing.T, dut *ondatra.DUTDevice, active, standby string) {
	...
	// synchronization; ContainerzSwitchoverReadyTimeoutS allows extra time for re-sync.
	timeout := 5 * time.Minute
	if s := deviations.ContainerzSwitchoverReadyTimeoutS(dut); s > 0 {
...
func doSwitchover(t *testing.T, dut *ondatra.DUTDevice, standby string) {
	...
	if s := deviations.ContainerzSwitchoverRetryTimeoutS(dut); s > 0 {
		retryTimeout = time.Duration(s) * time.Second
	}
